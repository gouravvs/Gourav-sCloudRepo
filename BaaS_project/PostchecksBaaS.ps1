# ============================================
# BAAS Node Migration Post-Check 
#Version 2.0
...
$OutputFile = "C:\Temp\BAAS_PostCheck_Report_{0}.csv" -f (Get-Date -Format "yyyyMMdd_HHmmss")
$LogFile = "C:\Temp\BAAS_PostCheck_{0}.log" -f (Get-Date -Format "yyyyMMdd_HHmmss")
...
{
    $WaveNo = (Read-Host "Please enter the Wave No for Post Checks").Trim()
    ...
    $NetworkRoot = "\\dbg.ads.db.com\Global-VFS\FRA\46965-1\CyberArk_st01626\CobaltIron_SQL\PostChecks"
...
Write-Log "=============================================="
Write-Log "Starting BAAS Node Migration Post-Check V2"
Write-Log "=============================================="
...
foreach ($Server in $Servers)
{
    ...
    $NewNodeName = $Server.NewNodeName.Trim()
    ...
    Write-Log "Expected Node     : $NewNodeName"
        ...
        Invoke-Command -ComputerName $ServerName -ScriptBlock {
            ...
            foreach($File in $Files)
            {
                ...
                if(Test-Path $Path)
                {
                    ...
                    if($Node)
                    {
                        return (($Node.Line -replace "^NODENAME\s+","").Trim())
                    }
                    else
                    {
...
#-------------------------------------------------------
# PHASE 3 - Validate Job History Node
#-------------------------------------------------------
$JobHistoryNode = ""
$TSMNodeStatus = "FALSE"
